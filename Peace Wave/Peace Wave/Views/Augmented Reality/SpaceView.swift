...
struct SpaceView: View {
    ...
    var body: some View {
        ARViewContainerSpace() // Container for AR view
    ...
    func playSound() {
        guard let soundURL = Bundle.main.url(forResource: "Space-Sound", withExtension: "mp3") else {
...
struct ARViewContainerSpace: UIViewRepresentable {
    static var arView: ARView? // Static reference to the AR view
    
    func makeUIView(context: Context) -> ARView {
        let arView = ARView(frame: .zero)
        ARViewContainerSpace.arView = arView
    ...
    func setupSkybox(for arView: ARView) {
        ...
        guard let skyboxURL = Bundle.main.url(forResource: "Space_Nebula", withExtension: "usdz") else {
