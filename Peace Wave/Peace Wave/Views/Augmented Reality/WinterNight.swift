...
struct WinterNight: View {
    ...
    func playSound() {
        guard let soundURL = Bundle.main.url(forResource: "Winter-Storm", withExtension: "mp3") else {
...
struct ARViewContainerWinterNight: UIViewRepresentable {
    static var arView: ARView? // Static reference to the AR view
    
    func makeUIView(context: Context) -> ARView {
        let arView = ARView(frame: .zero)
        ARViewContainerWinterNight.arView = arView
    ...
    func setupSkybox(for arView: ARView) {
        ...
        guard let skyboxURL = Bundle.main.url(forResource: "Winter_Night", withExtension: "usdz") else {
