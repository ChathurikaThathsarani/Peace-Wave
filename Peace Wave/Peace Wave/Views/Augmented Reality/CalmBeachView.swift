...
struct CalmBeachView: View {
    ...
    func playSound() {
        guard let soundURL = Bundle.main.url(forResource: "Calm-Beach", withExtension: "mp3") else {
...
struct ARViewContainerBeach: UIViewRepresentable {
    static var arView: ARView? // Static reference to the AR view
    
    func makeUIView(context: Context) -> ARView {
        let arView = ARView(frame: .zero)
        ARViewContainerBeach.arView = arView
    ...
    func setupSkybox(for arView: ARView) {
        ...
        guard let skyboxURL = Bundle.main.url(forResource: "Tropical_Island", withExtension: "usdz") else {
