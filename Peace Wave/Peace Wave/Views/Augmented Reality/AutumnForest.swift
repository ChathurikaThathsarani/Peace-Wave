...
struct AutumnForest: View {
    ...
    func playSound() {
        guard let soundURL = Bundle.main.url(forResource: "Autumn-Forest", withExtension: "mp3") else {
...
struct ARViewContainerForest: UIViewRepresentable {
    static var arView: ARView? // Static reference to the AR view
    
    func makeUIView(context: Context) -> ARView {
        let arView = ARView(frame: .zero)
        ARViewContainerForest.arView = arView
    ...
    func setupSkybox(for arView: ARView) {
        ...
        guard let skyboxURL = Bundle.main.url(forResource: "Autumn_Forest", withExtension: "usdz") else {
